...
const MensJewelry = () => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      // Make API request
      const response = await axios.get(`https://rmj-backend.onrender.com/api/products/category/men?${params}`);
    ...
    <div className={`mens-jewelry-listing ${pageLoaded ? 'fade-in' : ''}`}>
      ...
      <div className="mens-jewelry-filter-sort-container">
        <div className="mens-jewelry-filters">
          ...
          <div className="mens-jewelry-filter-dropdown">
            <button 
              className={`mens-jewelry-filter-btn ${categoryOpen ? 'active-filter' : ''}`}
            ...
            {categoryOpen && (
              <div className="mens-jewelry-dropdown-content">
                {['', 'Chain', 'Ring', 'Stud', 'Bracelet', 'Pendant','Kada'].map((category) => (
                  <label className="mens-jewelry-radio" key={category}>
                    <input
                      type="radio"
                      name="category"
                    ...
                    />
                    {category === '' ? 'All Categories' : category === 'Chain' ? 'Chains' : 
                     category === 'Ring' ? 'Rings' : category === 'Bracelet' ? 'Bracelets' : 
                     category === 'Pendant' ? 'Pendants' : category}
                  </label>
                ))}
              </div>
            )}
